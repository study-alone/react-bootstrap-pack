...
|                 |     |                                                                 |                                                           |                                                                                                               |
| --------------- | :-: | --------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `type`          | :x: | `'button' \| 'reset' \| 'submit'{:ts}`                          | tagName이 "button"인 경우에는 `button`이 기본값이 됩니다. | tagName이 botton인 경우에만 지정될 수 있고 다른 TagName인 경우 무시됩니다.                                    |
| `disabled`      | :x: | `boolean{:ts}`                                                  | `undefined`                                               | 비활성화 여부                                                                                                 |
| `role`          | :x: | `React.ArialRole{:ts}`                                          | `undefined`                                               | WAI-ARIA에 따른 role 속성                                                                                     |
